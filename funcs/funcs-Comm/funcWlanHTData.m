function [y,dataSpMapped] = funcWlanHTData(PSDU,cfgHT,varargin)
...
% BCC Interleaving
if strcmp(cfgHT.ChannelCoding,'BCC')
    interleavedData = wlanBCCInterleave(streamParsedData, 'VHT', numCBPSSI, chanBW);
else
...
packedData = complex(zeros(FFTLen, numSym, numSTS));
packedData(cfgOFDM.DataIndices,:,:) = stbcData;
packedData(cfgOFDM.PilotIndices,:,:) = pilots;
...
dataCycShift = wlan.internal.cyclicShift(rotatedData, csh, FFTLen);

% Spatial mapping
dataSpMapped = wlan.internal.spatialMap(dataCycShift, cfgHT.SpatialMapping, numTx, cfgHT.SpatialMappingMatrix);

% OFDM modulate
wout = wlan.internal.wlanOFDMModulate(dataSpMapped, CPLen);
