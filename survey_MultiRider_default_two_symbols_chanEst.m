...
snr = 20:10:40; %10:2:30;
global numTags;
numTags = 3;

refSyms = survey_MultiRider_funcGenerateReferenceSymbols(numTags);
...
%             payload = randi([0,1],numPayload,1);
        ...
        for rx_idx1 = 1:numTags
            ...
            channelEstimation{rx_idx1} = ofdmDataSymbols{rx_idx1}(:,1+(1+(rx_idx1-1)*2:rx_idx1*2));
        ...
        for rx_idx3 = 1:numTags
            ...
            A(:,2) = A(:,2).*(-1);
            B = ofdm_ref_channelestimation(:,1+(rx_idx3-1)*2:rx_idx3*2);
            tmp_LL = size(A,1);
            for idx_LL = 1:tmp_LL
                ...
                chanEst{rx_idx3}(idx_LL,1) = tmp_H;
            end
        end
        
        for rx_idx4 = 1:numTags
            tmp_rx_l2 = (rx_idx4-1)*interval+1;
            ...
            demodPayloadBits(:,rx_idx4) = survey_MultiRider_funcDemodulation(chanEst{rx_idx4}(1:tmp_distance),subcarriers_ref_tag,subcarriers_received_tag);
            % 重构干扰信号
            re_construct_interference_signal = survey_MultiRider_funcReConstructInterferenceSignal(demodPayloadBits(:,rx_idx4),ofdm_ref_data,chanEst{rx_idx4});
            % 消去干扰信号
            tmp_ref_l = (rx_idx4-1)*interval+1;
