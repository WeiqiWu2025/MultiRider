...
refSyms_addTags = survey_MultiRider_funcGeneratePreamble(seqLenForEstChannel,numTags+numAddTags);
refSyms(refSyms == 0) = 2;
refSyms(refSyms == 1) = 0;
refSyms_addTags(refSyms_addTags == 0) = 2;
refSyms_addTags(refSyms_addTags == 1) = 0;

global len_refSyms;
len_refSyms = size(refSyms_addTags,1);
...
global interval;
interval = ceil(numDataSubcarrier/(numTags+numAddTags));
...
for i = 1:S
    ...
    while  n<=maxNumPackets
        ...
        numPayload1 = numTagData - size(refSyms_addTags,1);
        for tag_idx2 = numTags+1:(numTags+numAddTags)
            tmp_payload1 = randi([0,1],numPayload1,1);
            actualPayloadBits_addTags(:,tag_idx2-numTags+1) = tmp_payload1;
            tagData(:,tag_idx2) = [refSyms_addTags(:,tag_idx2);tmp_payload1];
        end
        
        % backscatter modulation
        for tag_idx3 = 1:(numTags+numAddTags)
            bxSig{tag_idx3} = survey_MultiRider_funcBackscatter(exSig(:,tag_idx3),tagData(:,tag_idx3),1);
        end
        ...
        for chan_tag_rx_idx1 = 1:(numTags+numAddTags)
            ...
            bxSig{chan_tag_rx_idx1} = awgn(bxSig{chan_tag_rx_idx1},snr,'measured');
        end
        ...
        for rx_idx1 = 1:(numTags+numAddTags)
            ...
            ofdmCarriedData{rx_idx1} = ofdmDataSymbols{rx_idx1}(:,(1+len_refSyms)+(1:numPayload1));
        end
        ...
        [~,ofdmSymDerived] = survey_MultiRider_funcOFDMSymDerived(txPSDU,cfgHT);
        ...
        ofdm_ref_data = ofdmDataDerived(:,(1+len_refSyms)+(1:numPayload1));
        ofdm_ref_channelestimation = ofdmDataDerived(:,1+(1:len_refSyms));
        
        % Calculate channel coefficients
        for rx_idx3 = 1:(numTags+numAddTags)
            A = channelEstimation{rx_idx3};
            B = ofdm_ref_channelestimation(:,1+(rx_idx3-1)*seqLenForEstChannel:rx_idx3*seqLenForEstChannel);
            tmp_LL = size(A,1);
            for rx_idx4 = 1:tmp_LL
                ...
                chanEst{rx_idx3}(rx_idx4,1) = tmp_H;
            end
        end
        
        for rx_idx5 = 1:(numTags+numAddTags)
            tmp_rx_l2 = (rx_idx5-1)*interval+1;
            ...
            demodPayloadBits(:,rx_idx5) = survey_MultiRider_funcDemodulation(chanEst{rx_idx5}(1:tmp_distance),subcarriers_ref_tag,subcarriers_received_tag);
            % 重构干扰信号
            re_construct_interference_signal = survey_MultiRider_funcReConstructInterferenceSignal(demodPayloadBits(:,rx_idx5),ofdm_ref_data,chanEst{rx_idx5});
            % 消去干扰信号
            tmp_ref_l = (rx_idx5-1)*interval+1;
